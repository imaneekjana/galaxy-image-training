...
For self-supervised learning we use the same ResNet architecture as in the supervised case. However the training of the encoder is by infoNCE loss. The architecture and the training protocol is included in `simclr_module.py`. The class `ResNetSimCLR()` takes care of defining the loss function and training the given model constructed by `ResNet()`. The same script also contains supervised protocol `train_regressor()` for training a regressor model given the encoder (with frozen weights) and datasets. 

The encoder can be trained running the script `simclr_training.py`. The augmentations used are defined in this script and can be modified. Finally the script `regression_after_ssl.py` can be used to load the trained encoder and train (or load) the regressor and evaluate model performance in some chosen dataset.
