...
model_cnn = ResNetRegression(input_channels=5, emb_size=32, aux_features=4, out_dim=3)

#model_tr = ViTRegression(input_channels=5, emb_size=32, aux_features=4, out_dim=3)

model_cnn = ResNetRegression()

weights = torch.load('path/to/saved/model',map_location = 'cpu')
...
with torch.no_grad():
    for imgs, feats, targs in test_loader:
        ...
        preds_scaled = model_cnn(imgs, feats)
